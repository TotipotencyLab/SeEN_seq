#' Plot Enrichment from SummarizedExperiment Object
#'
#' This function generates enrichment plots from a SummarizedExperiment object containing enrichment data.
#'
#' @param SeEN_se SummarizedExperiment. The input object containing enrichment data.
#' @param x character. Column name in rowData to use as the x-axis (default: "motif_position").
#' @param y character or NULL. Assay name to use for the y-axis. If NULL, will be guessed using `y_regex`.
#' @param y_regex character. Regular expression to match assay names if `y` is NULL (default: "^enrichment_").
#' @param group_by character or NULL. Column name in colData to group data by (default: "sample").
#' @param color_by character or NULL. Column name in colData to color data by.
#' @param NA_replace.x value to replace NA values in the x variable (default: -1).
#' @param NA_replace.y value to replace NA values in the y variable.
#' @param show_plot_as character vector. Plot type(s) for main data ("line", "point"; default: "line"). For multiple plot types, user can use this argument as vector (i.e., `c("line", "point")`), or string (e.g., "line and point").
#' @param average_group logical. Whether to average values within groups (default: FALSE). If specified, the standard deviation of `y` value will be added to the plot
#' @param show_group_avg_as character vector. How should the standard deviation within group be shown (i.e., when `average_group` is `TRUE`) ("ribbon", "errorbar"; default: "ribbon").
#' @param facet_formula formula or character. Faceting formula for ggplot2 (e.g., "group ~").
#' @param show_gap.x logical. Whether to fill missing x positions with NA (default: TRUE). This will make the missing x become more apparent. The missing in x axis can occur sometimes in SeEN-seq experiments. For example, when motif insertion to the template match the restriction site of the restriction enzymes.
#' @param plot_data logical. If TRUE, returns the processed data frame instead of a plot (default: FALSE).
#'
#' @return A ggplot object showing the enrichment plot, or a tibble with processed data if `plot_data = TRUE`.
#'
#' @details
#' - Automatically selects the appropriate assay for plotting if not specified.
#' - Handles missing values in x and y axes.
#' - Supports grouping, coloring, and faceting.
#' - Can display group averages with ribbons or error bars.
#' - Warns if duplicated x values are detected within groups.
#'
#' @examples
#' # Assuming `se` is a SummarizedExperiment object with appropriate assays and metadata:
#' p <- SeEN_se.plot_enrichment(se, x = "motif_position", group_by = "sample", color_by = "condition")
#' print(p)
#'
...
SeEN_se.plot_enrichment <- function(SeEN_se, x = "motif_position", y = NULL, y_regex = "^enrichment_", group_by = "sample", color_by = NULL,
  ...
  if (is.null(y)) {
    ...
    if (length(y_candidates) == 0) {
      ...
      warning(
        "Multiple candidate assays found! Only using the first one.",
  ...
  if (!is.null(NA_replace.y)) {
    # Not sure whether this will be used
  ...
  # Average over group
  group_colnames <- unique(c(x, group_by, color_by))
  if (!is.null(facet_formula)) {
    # Get all variable names from the formula
    group_colnames <- unique(c(group_colnames, all.vars(facet_formula)))
    group_colnames <- group_colnames[group_colnames != "."] # exclude '.' that may come from formula
  }
  if (average_group) {
    # require(glue)
  ...
  if (show_gap.x) {
    # if specified, the missing position on the reference (i.e., rowData) will be filled with NA
    # Guess the step size on x
    x_step <- min(diff(sort(unique(plot_df[[x]]))), na.rm = TRUE)
    # Filling missing data on plot_df
    x_range <- seq(min(plot_df[[x]], na.rm = TRUE), max(plot_df[[x]], na.rm = TRUE), by = x_step)

    plot_df <- plot_df %>%
      group_by(across(all_of(group_colnames[group_colnames != x]))) %>%
      tidyr::complete(!!sym(x) := x_range) %>%
      ungroup() %>%
      # Rearrange columns
      dplyr::select(all_of(group_colnames), everything())
  }
