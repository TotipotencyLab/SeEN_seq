...
wd <- getwd() # assuming we are in the GitHub directory
config <- list(
...
config <- lapply(config, FUN = function(x) {
  ifelse(grepl("(_path$)|(_dir$)", x), yes = file_path_as_absolute(x), no = x)
})
...
if (length(invalid_lane_df_colnames) > 0) {
  message("These columns will be removed from the lane metadata table:\n ", paste0(invalid_lane_df_colnames, collapse = ", "))
}
...
tidy_enrich_df <- assay(SeEN_se, "enrichment_Bound_vs_Unbound") %>%
  ...
  tidyr::gather(key = "lane", value = "enrichment", -ref_name)
# Adding metadata columns from lane and reference table
tidy_enrich_df2 <- tidy_enrich_df %>%
  dplyr::left_join(tibble::rownames_to_column(lane_df, var = "lane"), by = "lane") %>%
  dplyr::left_join(tibble::rownames_to_column(ref_df, var = "ref_name"), by = "ref_name")
...
p <- enrich_df %>%
  ...
  geom_ribbon(aes(
    ymin = avg_enrichment - sd_enrichment,
...
result_list <- c(
  ...
  lapply(assays(SeEN_se), FUN = function(m) {
    tibble::rownames_to_column(as.data.frame(m), var = "sample")
  }),
  list(
